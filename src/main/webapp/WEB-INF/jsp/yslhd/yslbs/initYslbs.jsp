...
<table  class="table table-bordered table-striped" style="width: 98%; margin:0px auto;">
    ...
    <tr>
        <td colspan="2">
            <a href="#">
                <button id="sybs" type="button" class="btn btn-success btn-sm" data-toggle="modal"  onclick="getSyModel();">
                    <i class="glyphicon glyphicon-plus"  onclick="getSyModel();"></i>报送
                </button>
            </a>
...
<!--4.选择水源列表-->
<div class="modal" id="syModel" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div style="width:1100px;" class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="myModalLabel">水源列表</h4>
            </div>
            <div class="modal-body">
                <div class="ibox-content">
                    <form id="sylbForm" method="post" class="form-horizontal">
                        <input type="hidden" name="syqydm"/>
                        <input type="hidden" name="optType"/>
                        <div class="form-group">
                            <label class="col-sm-3 control-label">取水口所在地</label>
                            <div class="col-sm-8">
                                <select id="qskszd"  class="select-qskszd col-sm-10" style="width: 100%" name="qskszd"></select>
                            </div>
                        </div>
                        <div class="text-right">
                            <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                            <button type="button" class="btn btn-primary sylbSave">确定</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div class="modal-dialog" style="width: 1200px;height: 600px">
        <div class="modal-content">
            ...
            <div class="tab-content">
                <!--图片上传-->
                <div role="tabpanel" class="tab-pane active" id="ckmx">
                    <div class="tab-content">
                        <div class="modal-body">
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        //水源列表绑定事件
        $(".sylbSave").click(function(){
            var _qskszd = $("#qskszd").select2("data");
            var sybh = "";
            if (_qskszd != null && JSON.stringify(_qskszd) != "{}" && typeof(_qskszd) != "undefined" && _qskszd.length > 0) {
                sybh = _qskszd[0].id;
            }
            window.location.href = "${ctx}/yslhd/yslbs/initYslbsAdd?sybh=" + sybh;
            $("#sybs").attr("data-target","");
        });
    ...
    function getSyModel(){
        $.ajax({
            ...
            url: '${ctx}/yslhd/yslbs/getSyList',
            type: 'post',
            dataType: 'json',
            success: function (data) {
                if(data.length==1){
                    window.location.href = "${ctx}/yslhd/yslbs/initYslbsAdd?sybh=" + data[0].id;
                }else if(data.length>1){
                    $("#sybs").attr("data-target","#syModel");
                    $(".select-qskszd").select2({
                        data: data,
                        placeholder: '请选择',
                        allowClear: true
                    });
    ...
    //用水量情况
    $("#myview").on('shown.bs.modal',function(e){

       // console.log($(e.relatedTarget).data());

        var id=$(e.relatedTarget).data("row");
       // console.log($(e.relatedTarget).data("row"));
    ...
    function myviewdata(xxcjid) {
        $.ajax({
            contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
            url: '${ctx}/yslhd/yslbs/doQueryTable',
            type: 'post',
            dataType: 'json',
            data: {
                'xxcjid': xxcjid
    ...
    function inittabs(bssjq, bssjz) {
        ...
        $('#taskList_table').bootstrapTable({
            url: "${ctx}/yslhd/yslbs/selectAll",//获取数据的Select地址
            ...
            queryParams: function (params) {
                ...
                if(bssjq!=null&&bssjz!=null){
                    param['svo.bssjq']=bssjq;
                    param['svo.bssjz']=bssjz;
                }
                return param;
            },
            onLoadSuccess:function(data){
              //  console.log("jdfklj"+JSON.stringify(data));
            },
            ...
            columns: [{
                ...
                align: 'center'
                    ...
                    formatter: function (value, row, index) {
                        return '<button  type="button" name="myviewBtn"  class="btn btn-link myviewBtn" data-toggle="modal" data-target="#myview" data-row="'+row.xxcjid+'"  >查看明细</button>'
                    }
                }]
        });
    }
    ...
    function btn_edit() {
        ...
        if (rows.length == 0) {
            ...
            var hdjglx = rows[0].hdjglxmc.trim();
            if(hdjglx=='已核定已通过'){
                layer.alert("该水源已报送核定不能编辑！");
                return;
            }else{
                window.location.href = "${ctx}/yslhd/yslbs/initYslbsEdit?xxcjid=" + rows[0].xxcjid;
            }
        }
    }
