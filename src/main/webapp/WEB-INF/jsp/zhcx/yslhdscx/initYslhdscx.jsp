...
<table class="table table-bordered table-striped" style="width: 98%; margin:20px auto;">
    ...
    <tr>
        <td>报送时间起:</td>
        <td>
            <div id="hdsjqs" class="input-group date form_date col-sm-5"
                 data-date-format="yyyy/mm/dd"
                 data-link-format="yyyy/mm/dd">
                <input id="qsssqq" class="form-control" type="text" name="qsssqqjy"
                       readonly>
        ...
        <td>报送时间止:</td>
        <td>
            <div id="hdsjzs" class="input-group date form_date  col-sm-5"
                 data-date-format="yyyy/mm/dd"
                 data-link-format="yyyy/mm/dd">
                <input id="qsssqz" class="form-control" type="text" name="qsssqzjy"
                       readonly>
                    <span class="input-group-addon">
                        <span class="glyphicon glyphicon-calendar"></span>
                    </span>
    ...
    <tr>
        <td>纳税人识别号:</td>
        <td><input id="nsrsbh" type="text"  style="width: 220px;"  class=" form-control "></td>
        <td>纳税人名称:</td>
        <td><input id="nsrmc" type="text"   style="width: 220px;" class="form-control "></td>
    </tr>
    ...
    <tr>
        ...
        <td>水源类型:</td>
        <td><select class="input-sylx" style="width: 220px;" id="sylxjy" name="sylxjy"></select></td>
    </tr>

    <tr>
        <td>核定书编号:</td>
        <td colspan="3"><input id="hdsbh" type="text"   style="width: 220px;" class="form-control "></td>
    </tr>
    <tr>
    ...
    <tr>
        <td colspan="2">
            <button type="button" class="btn btn-warning btn-sm" onclick="btn_edit()"><i
                    class="glyphicon glyphicon-pencil" onclick="btn_edit()"></i>导出核定书
            </button>
        </td>
    </tr>
    </tbody>
...
<script type="text/javascript">
    $(function () {
        ...
        //水源类型（下拉菜单）
        $.post("${ctx}/zhcx/yslhdscx/sylxcd", null, function (data) {
            $(".input-sylx").select2({
                data: data,
                placeholder: '请选择',
                allowClear: true
        ...
        //水源区域（下拉菜单）
        $.post("${ctx}/zhcx/yslhdscx/syqycd", null, function (data) {
            $(".input-syqy").select2({
                data: data,
                placeholder: '请选择',
                allowClear: true
    ...
    function myviewdata(xxcjid) {
        $.ajax({
            ...
            url: '${ctx}/zhcx/yslhdscx/doQueryTable',
            type: 'post',
            dataType: 'json',
            data: {
                'xxcjid': xxcjid
    ...
    function searchlist() {
        //取水所属期起
        var qsssqq = $("#qsssqq").val().trim();
        //取水所属期止
        var qsssqz = $("#qsssqz").val().trim();
        //纳税人识别号
        var nsrsbh = $("#nsrsbh").val().trim();
        //纳税人名称
        var nsrmc = $("#nsrmc").val().trim();
        //核定书编号
        var hdsbh =$("#hdsbh").val().trim();
        //水源区域
        var syqy = $("#syqy").select2("data")[0].id=='00'?'%':$("#syqy").select2("data")[0].id;
        //水源类型
        var sylx = $("#sylxjy").select2("data")[0].id=='00'?'%':$("#sylxjy").select2("data")[0].id;

        //先销毁表格
        $('#taskList_table').bootstrapTable('destroy');
        $('#taskList_table').bootstrapTable({
            url: "${ctx}/zhcx/yslhdscx/selectAll",//获取数据的Select地址
    ...
    function btn_edit() {
        ...
        if (rows.length == 0) {
            layer.alert("请选择要查看核定书的数据！");
            return;
        } else {
            window.location.href = "${ctx}/zhcx/yslhdscx/initYslhds?xxcjid=" + rows[0].xxcjid;
        }
    }
